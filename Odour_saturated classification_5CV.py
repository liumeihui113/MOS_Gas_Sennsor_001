...
# -------------------------- 1. Data Loading from 5 pre-split folders --------------------------
DATA_ROOT = r'Odour_saturated_dataset_5 batches'   # Contains subfolders 1,2,3,4,5
print("=" * 60)
print("Loading data from pre-split folders...")
print("=" * 60)
...
# Collect all files with their fold ID and class prefix
all_files = []  # each element: (fold_id, file_path, class_prefix)
for fold_name in ['1', '2', '3', '4', '5']:
...
# Group by class to validate completeness
class_file_dict = {}
...
if sorted_class_prefixes != [str(i) for i in range(1, 101)]:
    raise ValueError(f"Incomplete classes, found: {sorted_class_prefixes}, need 1-100")
...
for cls in sorted_class_prefixes:
    ...
    print(f"Class {cls}: {fold_counts}")  # should show 2 samples per fold for each class

# Build data arrays
class_label_map = {cls: idx for idx, cls in enumerate(sorted_class_prefixes)}
label_class_map = {idx: cls for cls, idx in class_label_map.items()}

data_list, label_list, fold_list = [], [], []
# Sort by class then by fold for deterministic order
all_files_sorted = sorted(all_files, key=lambda x: (int(x[2]), x[0]))

for fold_id, fpath, cls in all_files_sorted:
    df = pd.read_excel(fpath).iloc[:, 1:]   # skip first column (index)
...
data = np.array(data_list)          # shape: [1000, T, 24]
labels = np.array(label_list)       # shape: [1000]
folds = np.array(fold_list)         # shape: [1000], values 1~5

print(f"\nTotal samples: {len(data)} (100 classes, 10 each)")
print(f"Sample shape: {data.shape[1:]} (time steps × channels)")
print(f"Label distribution: {np.bincount(labels)}")
print(f"Samples per fold: {[np.sum(folds==i) for i in range(1,6)]}")
print("=" * 60)
...
def train_and_evaluate(fold_id, train_indices, test_indices, data_tensor, labels_tensor,
                       result_root, device, epochs=300, batch_size=64):
    """
    Train and evaluate on one fold (one folder as test set).
    Saves results in result_root/fold_{fold_id}/
    Returns test targets, predictions, and probabilities.
    """
    fold_dir = os.path.join(result_root, f'fold_{fold_id}')
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % 10 == 0:
            print(f"Fold {fold_id} Epoch {epoch+1:3d} | Train Loss: {train_loss:.4f} Acc: {train_acc:.4f} | Test Loss: {test_loss:.4f} Acc: {test_acc:.4f}")
    ...
    metrics_str = (f"Fold {fold_id} Test Metrics\n"
                   f"Accuracy: {acc:.4f}\n"
    ...
    plt.title(f'Fold {fold_id} Test Confusion Matrix')
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'Fold {fold_id} Loss Curves')
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title(f'Fold {fold_id} Accuracy Curves')
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.title(f'Fold {fold_id} PR Curves')
...
# -------------------------- 4. Main: 5-fold Cross Validation (leave-one-folder-out) --------------------------
if __name__ == "__main__":
    ...
    RESULT_ROOT = 'Odour_saturated_results_100_5CV_leave_folder_out'
    os.makedirs(RESULT_ROOT, exist_ok=True)

    fold_results = []  # store (targets, preds, probs) for each fold

    # Loop over each folder as test set
    for test_fold in [1, 2, 3, 4, 5]:
        print(f"\n{'='*60}\nStarting Fold {test_fold}: Test set = Folder {test_fold}\n{'='*60}")
    ...
    print("\n" + "="*60)
    print("Summary of 5-fold Cross-Validation (leave-one-folder-out)")
    print("="*60)
